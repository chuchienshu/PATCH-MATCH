...
float nn_search_gpu( uchar4 *first, int fWidth, int fHeight, uchar4 *second, int sWidth, int sHeight, mapent *curMap, mapent *newMap ) {
  ...
  checkCudaErrors( cudaMalloc( (void **)&R_dev, rBytes ) );

  if ( NNF_DEBUG ){ printf( "Allocating mem for image 1...\n" ); fflush(stdout); }
  checkCudaErrors( cudaMalloc( (void **)&F_dev, fBytes ) );

  if ( NNF_DEBUG ){ printf( "Copying image 1 data to device...\n" ); fflush(stdout); }
  checkCudaErrors( cudaMemcpy( F_dev, first, fBytes, cudaMemcpyHostToDevice ) );
  ...
  checkCudaErrors( cudaMalloc( (void **)&curMap_dev, mBytes ) );
  checkCudaErrors( cudaMalloc( (void **)&newMap_dev, mBytes ) );

  if ( NNF_DEBUG ){ printf( "Copying map data to device...\n" ); fflush(stdout); }
  checkCudaErrors( cudaMemcpy( curMap_dev, curMap, mBytes, cudaMemcpyHostToDevice ) );
  //checkCudaErrors( cudaMemcpy( newMap_dev, newMap, mBytes, cudaMemcpyHostToDevice ) );

  // move the params into constant memory
  if ( NNF_DEBUG ){ printf( "Copying parameters to constants...\n" ); fflush(stdout); }
  checkCudaErrors( cudaMemcpyToSymbol( C_fW, &fWidth,  sizeof(   int ) ) );
  checkCudaErrors( cudaMemcpyToSymbol( C_fH, &fHeight, sizeof(   int ) ) );
  checkCudaErrors( cudaMemcpyToSymbol( C_sW, &sWidth,  sizeof(   int ) ) );
  checkCudaErrors( cudaMemcpyToSymbol( C_sH, &sHeight, sizeof(   int ) ) );
  checkCudaErrors( cudaMemcpyToSymbol( C_hp, &halfP,   sizeof(   int ) ) );
  checkCudaErrors( cudaMemcpyToSymbol( C_ps, &patch,   sizeof(   int ) ) );
  // checkCudaErrors( cudaMemcpyToSymbol( (const char*)"C_fW", &fWidth,  sizeof(   int ) ) );
  // checkCudaErrors( cudaMemcpyToSymbol( (const char*)"C_fH", &fHeight, sizeof(   int ) ) );
  // checkCudaErrors( cudaMemcpyToSymbol( (const char*)"C_sW", &sWidth,  sizeof(   int ) ) );
  // checkCudaErrors( cudaMemcpyToSymbol( (const char*)"C_sH", &sHeight, sizeof(   int ) ) );
  // checkCudaErrors( cudaMemcpyToSymbol( (const char*)"C_hp", &halfP,   sizeof(   int ) ) );
  // checkCudaErrors( cudaMemcpyToSymbol( (const char*)"C_ps", &patch,   sizeof(   int ) ) );

  // deploy texture cache
  checkCudaErrors( cudaMallocArray( &dca_s, &dtl_s.channelDesc, sWidth, sHeight ) );

  // copy input data to cuda array
  checkCudaErrors( cudaMemcpyToArray( dca_s, 0, 0, second, sBytes, cudaMemcpyHostToDevice ) );
  ...
  checkCudaErrors( cudaBindTextureToArray( dtl_s, dca_s ) ); //
  ...
  for ( int i = 1; i <= ITERATIONS; i++ ) {
    ...
    checkCudaErrors( cudaMemcpy( R_dev, rands, rBytes, cudaMemcpyHostToDevice ) );
    ...
    /* only to test the PRNG
       ...
       checkCudaErrors( cudaMemcpy( rands, R_dev, rBytes, cudaMemcpyDeviceToHost ) );
    ...
    if ( NNF_VIZ ) {
      char fname[64];
      sprintf( fname, "gpu-iter-%i.jpg", i );

      //cutilCheckError( cutStopTimer( timer ) );
      checkCudaErrors( cudaMemcpy( newMap, newMap_dev, mBytes, cudaMemcpyDeviceToHost ) );
      nn_viz_map( fWidth, fHeight, second , sWidth, sHeight, newMap, fname );
      //cutilCheckError( cutStartTimer( timer ) );
    }

    // move the results - for more processing
    //checkCudaErrors( cudaMemcpy( curMap_dev, newMap_dev, mBytes, cudaMemcpyDeviceToDevice ) );
    temp = curMap_dev;
  ...
  checkCudaErrors( cudaFree( R_dev ) );
  checkCudaErrors( cudaFree( F_dev ) );
  checkCudaErrors( cudaFree( curMap_dev ) );
  checkCudaErrors( cudaFree( newMap_dev ) );
  checkCudaErrors( cudaUnbindTexture( dtl_s ) );
  checkCudaErrors( cudaFreeArray( dca_s ) ); //
